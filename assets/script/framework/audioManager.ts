import { _decorator, Component, Node, AudioClip, AudioSource, game, director } from "cc";
import { mergeWriteClientPrefs } from "./clientPrefs";
...
export class AudioManager {
    ...
    getAudioSetting(isMusic: boolean) {
        ...
        const key = isMusic ? 'musicOn' : 'soundOn';
        return pi[key] !== false;
    }
    ...
    openMusic() {
        this.setMusic(0.8);
        mergeWriteClientPrefs({ musicOn: true });
        if (playerData.instance.playerInfo) {
            playerData.instance.updatePlayerInfo('musicOn', true);
        }
    }

    closeMusic() {
        this.setMusic(0);
        mergeWriteClientPrefs({ musicOn: false });
        if (playerData.instance.playerInfo) {
            playerData.instance.updatePlayerInfo('musicOn', false);
        }
    }

    openSound() {
        this.setSound(1);
        mergeWriteClientPrefs({ soundOn: true });
        if (playerData.instance.playerInfo) {
            playerData.instance.updatePlayerInfo('soundOn', true);
        }
    }

    closeSound() {
        this.setSound(0);
        mergeWriteClientPrefs({ soundOn: false });
        if (playerData.instance.playerInfo) {
            playerData.instance.updatePlayerInfo('soundOn', false);
        }
    }
