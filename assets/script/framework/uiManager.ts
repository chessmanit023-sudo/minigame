import { _decorator, Component, Node, SpriteComponent, Color, RichTextComponent, find, isValid, Vec3 } from "cc";
...
export class uiManager {
    ...
    public showDialog (panelPath: string, args?: any, cb?: Function) {
        ...
        if (this._dictSharedPanel.hasOwnProperty(panelPath)) {
            ...
            if (isValid(panel)) {
                ...
                // 刷新多语言（对已缓存的面板进行多语言适配）
                i18nManager.instance.refreshNode(panel);
        ...
        resourceUtil.createUI(panelPath, (err: any, node: any) => {
            ...
            // 刷新多语言（对动态加载的预制体进行多语言适配）
            i18nManager.instance.refreshNode(node);
     ...
     * @param {Function} cb 回调函数，创建完毕后回调
     */
    public showDialogNoCache (panelPath: string, args?: any, cb?: Function) {
        if (this._dictLoading[panelPath]) {
            return;
        }
