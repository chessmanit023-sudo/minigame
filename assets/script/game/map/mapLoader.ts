...
const MAP_DATA_FIRST = '编号,对应资源编号,位置,缩放,旋转X,旋转Y,旋转Z,预制体UUID' + '\n' +
    'number,string,string,string,string,string,string,string' + '\n' +
    'ID,name,position,scale,eulX,eulY,eulZ,prefabUuid' + '\n';
/*
...
export class MapLoader extends Component {
    ...
    private _saveMap() {
        ...
        for (let i = 0; i < this.node.children.length; i++) {
            ...
            const ndName = this._getExportItemName(ndItem);
            ...
            // 获取欧拉角（worldEulerAngles会自动计算，如果localEulerAngles不对可以尝试world）
            const eulX = this._getNumberToFixed(ndItem.eulerAngles.x, 2);
            const eulY = this._getNumberToFixed(ndItem.eulerAngles.y, 2);
            const eulZ = this._getNumberToFixed(ndItem.eulerAngles.z, 2);
            const prefabUuid = this._getExportPrefabUuid(ndItem);
            //生成sting型数据  数据之间以,隔开 在最后加上换行\n
            let itemData = `${i + 1},${ndName},${pos},${scale},${eulX},${eulY},${eulZ},${prefabUuid}` + '\n';
            data += itemData;
        }
    ...
    /**
     * 导出地图物件名。
     * 运行期道路仍使用数字编号，建筑则优先使用其 prefab 原始资源名，
     * 避免策划在场景里把同一个 prefab 改成 Building_08 这类实例名后，
     * 导出的名字和真实资源对不上。
     */
    private _getExportItemName(ndItem: Node) {
        ...
        const prefabInfo = (ndItem as any)['_prefab'];
        const prefabAsset = prefabInfo?.asset;
        return prefabAsset?.name || prefabAsset?._name || ndName;
    }

    private _getExportPrefabUuid(ndItem: Node) {
        const prefabInfo = (ndItem as any)['_prefab'];
        const prefabAsset = prefabInfo?.asset;
        return prefabAsset?._uuid || prefabAsset?.uuid || '';
    }
    /**
     * 将Vec3类型数据保留小数点后两位并
     * @returns string 
     ...
     * @returns number
     */
    private _getNumberToFixed(num: number, digit: number = 2) {
    ...
    private _setLoadDataToMap(nowData: any) {
        ...
        while (dataList[i]) {
            ...
            //举例：itemData = ["1,roadStraight3,", "0.00,0.00,1.15", "1.00,1.00,1.00", "0", "0", "0"]
            //                 id     name              position        scale       eulX  eulY  eulZ
            const nameList = this._deleteUselessData(itemData[0].split(','));
            //举例：nameList = ["1", "roadStraight3"]
            //                 id     name   

            const eulX = Number(itemData[3]);
            const eulY = Number(itemData[4]);
            const eulZ = Number(itemData[5]);
            ...
            ndItem?.setRotationFromEuler(new Vec3(eulX, eulY, eulZ));
